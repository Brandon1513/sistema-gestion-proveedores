...
class ProviderTypeController extends Controller
{
    ...
    public function show(ProviderType $providerType): JsonResponse
    {
        $providerType->load(['documentTypes' => function ($query) {
            $query->withPivot(['is_required', 'sort_order', 'applies_to_existing'])
                  ->orderBy('category')
                  ->orderBy('name');
        }]);
    ...
    /**
     * Obtener documentos asignados con detalle completo (para gestión)
    ...
    public function documents(ProviderType $providerType): JsonResponse
    {
        ...
        // Documentos disponibles para asignar (no asignados aún)
    ...
    public function assignDocument(Request $request, ProviderType $providerType): JsonResponse
    {
        ...
        if ($exists) {
            return response()->json(['message' => 'Este documento ya está asignado a este tipo de proveedor'], 422);
        }
